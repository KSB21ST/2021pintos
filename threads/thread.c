...
thread_create (const char *name, int priority,
   ...
   /*allocate file descriptor table for thread t. Allocate it with palloc_get_page,
   although this might be excessive amount of 4KB allocated.
   free it in process_exit().
   */
   t->fd_table = palloc_get_page(PAL_ZERO);
   if(t->fd_table == NULL) /*if there's not enough memeory for fd_table, reuturn TID_ERROR*/
      return TID_ERROR;
   /*
   in file descriptor table,
   0 stands for STDIN, 1 stands for STDOUT according to KAIST PINTOS GITBOOK.
   originally, we wouldn't care about fd_table[0] and fd_table[1].(fd = 0, fd = 1)
   but dup2 can change the file descriptor to point at STDIN and STDOUT even if it's not 0 and 1.
   So, we allocate -1 (STDIN) and -2 (STDOUT) to show that if fd_table[fd] = -1, it's(fd) pointing at STDIN.
   */
   t->fd_table[0] = -1;
   t->fd_table[1] = -2;
...
init_thread (struct thread *t, const char *name, int priority) {
   ...
   /*for proj2 - detailed explanation in thread.h
   careful about making big threads.
   thread size should be beneath 1KB.
   if you want to check, printf("thread size: %d \n", sizeof(struct thread)); in thread_create().
   */
