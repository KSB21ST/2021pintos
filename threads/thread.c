...
void increment (void);
void cal_all_recent_cpu (void);
void cal_all_priority (void);
...
thread_get_recent_cpu (void) {
   ...
   int ans = thread_current()->recent_cpu * 100;
   if(ans >= 0){
	   ans = (ans + f / 2) / f;
   }else{
	   ans = (ans - f / 2) / f;
...
void cal_priority (struct thread *t){
   if (t != idle_thread){
	  ...
	  int new_priority = PRI_MAX - temp1 - temp2;
...
void cal_load_avg (void){
   ...
   int temp1 = (59 * f) / 60;
   temp1 = (int64_t)temp1 * load_avg / f;	// (59/60) * load_avg
   int temp2 = (1 * f) / 60;
   temp2 *= ready_threads;
   int new_load_avg = temp1 + temp2;		// (1/60) * ready_threads
   ...
   if(!list_empty(&all_list)){
      ...
      for (e = list_begin (&all_list); e != list_end (&all_list); e = list_next (e)) {
          struct thread *t = list_entry(e, struct thread, allelem);
			if (t != idle_thread){
				int new_recent_cpu = ((int64_t)(load_avg * 2)) * f / (load_avg * 2 + 1 * f);	// (2 * load_avg) / (2 * load_avg + 1)
				new_recent_cpu = ((int64_t)new_recent_cpu) * (t->recent_cpu) / f + (t->nice * f);
